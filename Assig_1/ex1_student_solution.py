...
class Solution:
    ...
    def compute_homography_naive(match_p_src: np.ndarray,
        ...
        # return homography
        P=[]
        for i, (xs, ys, xd, yd) in enumerate(zip(match_p_src[0], match_p_src[1], match_p_dst[0], match_p_dst[1])):
            P.append(np.array([xd, yd, 1, 0, 0, 0, -xd * xs, -yd * xs, -xs]))
            P.append(np.array([0, 0, 0, xd, yd, 1, -ys * xd, -ys * yd, -ys]))
        P = numpy.asarray(P) #full 2nx9 matrix

        u, s, vh = np.linalg.svd(P)
        H = vh[-1,:]# 1x9 homography H1T;H2T;H3T
        expiriment_eigen_value = numpy.matmul(P, H)  # sanity check

        z = cv2.findHomography(match_p_src.T,match_p_dst.T)
