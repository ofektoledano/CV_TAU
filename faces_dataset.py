...
class FacesDataset(Dataset):
    ...
    def __getitem__(self, index) -> tuple[torch.Tensor, int]:
        ...
        if index<len(self.real_image_names):
            img_name = os.path.join('real',self.real_image_names[index])
            label = 0
        else:
            img_name = os.path.join('fake', self.fake_image_names[index-len(self.real_image_names)])
            label = 1
        image = Image.open(os.path.join(self.root_path,img_name),'r')
